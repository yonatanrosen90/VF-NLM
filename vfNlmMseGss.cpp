...
#include <Rcpp.h>
#include <math.h>
//#include "/sci/home/rosen90/thesis/chrom21/vfNlmMse.cpp"
#include "C:/Users/yonatan/Documents/university/thesis/rcode/chrom21/vfNlmMse.cpp"

using namespace Rcpp;

// [[Rcpp::export]]

//golden section search algorithm for vf-nlm

List vf_nlf_mse_gss(NumericVector x, NumericVector true_y, NumericVector sig,
	...
	do {
		...
		double mid_h1_val = vf_nlf_mse(x, mid_h1, best_a, true_y, sig)["mse"];
		double mid_h2_val = vf_nlf_mse(x, mid_h2, best_a, true_y, sig)["mse"];
		...
		while ( abs(max_h_rg-min_h_rg)>tol_params and h_iter<MaxIter_params ) {
			...
			if (mid_h2_val > mid_h1_val){
				...
				mid_h1_val = vf_nlf_mse(x, mid_h1, best_a, true_y, sig)["mse"];
			} else {
				min_h_rg = mid_h1;
				mid_h1 = mid_h2;
				mid_h1_val = mid_h2_val;
				mid_h2 = max_h_rg - (max_h_rg - min_h_rg)*rho;
				mid_h2_val = vf_nlf_mse(x, mid_h2, best_a, true_y, sig)["mse"];
			}
		...
		double mid_a1_val = vf_nlf_mse(x, best_h, mid_a1, true_y, sig)["mse"];
		double mid_a2_val = vf_nlf_mse(x, best_h, mid_a2, true_y, sig)["mse"];
		...
		while ( abs(max_a_rg-min_a_rg)>tol_params and a_iter<MaxIter_params) {
			...
			if (mid_a2_val > mid_a1_val){
				...
				mid_a1_val = vf_nlf_mse(x, best_h, mid_a1, true_y, sig)["mse"];
			} else {
				min_a_rg = mid_a1;
				mid_a1 = mid_a2;
				mid_a1_val = mid_a2_val;
				mid_a2 = max_a_rg - (max_a_rg - min_a_rg)*rho;
				mid_a2_val = vf_nlf_mse(x, best_h, mid_a2, true_y, sig)["mse"];
			}
			
		}
		best_a = min_a_rg * 0.5 + max_a_rg * 0.5;
		
		best_res = vf_nlf_mse(x, best_h, best_a, true_y, sig);
		mse = best_res["mse"];
	}
